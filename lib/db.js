// lib/db.js
// ชั้นเก็บข้อมูล:
// - สินค้า (products) → เก็บใน Supabase Postgres table "products" (แก้ให้ทำงานได้จริงบน Vercel
//   เพราะระบบไฟล์บน Vercel serverless เป็น read-only/ephemeral เขียนไฟล์ data/products.json
//   แล้วข้อมูลจะหายเมื่อ deploy ใหม่หรือ instance ถูกรีไซเคิล)
...
import { supabaseAdmin, PRODUCTS_TABLE } from "./supabase";
...
function productToRow(product, { partial = false } = {}) {
  ...
  for (const [key, column] of Object.entries(map)) {
    ...
    if (column === "original_price" && (value === "" || value === undefined)) {
      value = null;
    }
...
export async function getPublishedProducts() {
  const products = await getAllProducts();
  return products.filter((p) => p.published !== false).map(normalizeProduct);
}
